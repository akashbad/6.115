;   *************************************************
;   *  Akash Badshah, Lab 3, Exercise 1             *
;   *   Modified to include T command               *
...
jumtab:
   ...
   .dw readaddr           ; command 'r' 12 used
   .dw badcmd             ; command 's' 13
   .dw tablewrite         ; command 't' 14 used
   .dw badcmd             ; command 'u' 15
   .dw badcmd             ; command 'v' 16
   .dw writeaddr          ; command 'w' 17 used
...
gb_err:
   ljmp badpar
;===============================================================
; readaddr 'r' - this routine reads the data from the 4 hex digit
; address which follows and prints it to the screen
;===============================================================
readaddr:
  lcall getbyt            ; get address high byte
  mov dph, a              ; save it in dph
  lcall prthex            ; print back the high byte
  lcall getbyt            ; get address low byte
  mov dpl, a              ; save it in dpl
  lcall prthex            ; print back the low byte
  lcall crlf              ; print a newline
  movx a, @dptr           ; move the value from the address to acc
  lcall prthex            ; print out the data from a, which is what we want
  ljmp endloop            ; return
;===============================================================
; writeaddr 'w' - this routine writes to the address specified by
; the 4 hex digit address which follows the data specified by the
; following '=' and 2 hex digit number
;===============================================================
writeaddr:
  lcall getbyt            ; get address high byte
  mov dph, a              ; save it in dph
  ...
  mov dpl, a              ; save it in dpl
  lcall prthex            ; print back the low byte
  lcall getchr            ; get the '=' character
  ...
  ljmp endloop            ; return
;===============================================================
; tablewrite 't' - this routine writes a table of up to 256 bytes
; to the set of hex addresses starting with the 4 digit address
; following the 'T' command. It stops if the user enters a return
; before the 256 bytes are written, it uses r0 to count 256
;===============================================================
tablewrite:
  lcall getbyt            ; get address high byte
  mov dph, a              ; save it in dph
  ...
  mov dpl, a              ; save it in dpl
  lcall prthex            ; print back the low byte
  mov r0, #0              ; counting with 256 bytes
tablewrite_loop:
  lcall crlf              ; send off the newline character
  mov a, dph              ; move the high byte of the dp to be printed
  lcall prthex            ; print it back
  mov a, dpl              ; move the low byte of the dp to be printed
  lcall prthex            ; print it back
  mov a, #' '             ; move the space into the accumulator
  lcall sndchr            ; send the space
  lcall getbyt            ; get the byte they want in th location
  movx @dptr, a           ; put that byte in memory at the dp location
  lcall prthex            ; show them the byte they just gave
  inc dptr                ; increment the data pointer to  
  djnz r0, tablewrite_loop; count off 256 of these
  ljmp endloop            ; finish the loop
;*****************************************************************
