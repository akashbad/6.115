/*******************************************************************************
* File Name: CapSense_CSD_IntClock.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_Start
********************************************************************************
*
* Summary:
...
void CapSense_CSD_IntClock_Start(void) 
{
    ...
    CapSense_CSD_IntClock_CLKEN |= CapSense_CSD_IntClock_CLKEN_MASK;
	CapSense_CSD_IntClock_CLKSTBY |= CapSense_CSD_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_Stop
********************************************************************************
*
* Summary:
...
void CapSense_CSD_IntClock_Stop(void) 
{
    ...
    CapSense_CSD_IntClock_CLKEN &= (uint8)(~CapSense_CSD_IntClock_CLKEN_MASK);
	CapSense_CSD_IntClock_CLKSTBY &= (uint8)(~CapSense_CSD_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CapSense_CSD_IntClock_CLKEN & CapSense_CSD_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CapSense_CSD_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CapSense_CSD_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CapSense_CSD_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CapSense_CSD_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CapSense_CSD_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CapSense_CSD_IntClock_GetSourceRegister();
    uint16 oldDivider = CapSense_CSD_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CapSense_CSD_IntClock_CLKEN & CapSense_CSD_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CapSense_CSD_IntClock_DIV_PTR, clkDivider);
                CapSense_CSD_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CapSense_CSD_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CapSense_CSD_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CapSense_CSD_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CapSense_CSD_IntClock_DIV_PTR, clkDivider);
				CapSense_CSD_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CapSense_CSD_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CapSense_CSD_IntClock_MOD_SRC |= modeBitMask & (uint8)CapSense_CSD_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CapSense_CSD_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CapSense_CSD_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CapSense_CSD_IntClock_MOD_SRC & (uint8)(CapSense_CSD_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CapSense_CSD_IntClock_GetDividerRegister();
    uint8 oldSrc = CapSense_CSD_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CapSense_CSD_IntClock_MOD_SRC |= CYCLK_SSS;
        CapSense_CSD_IntClock_MOD_SRC =
            (CapSense_CSD_IntClock_MOD_SRC & (uint8)(~CapSense_CSD_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CapSense_CSD_IntClock_MOD_SRC & CapSense_CSD_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CapSense_CSD_IntClock_PHASE = clkPhase & CapSense_CSD_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CapSense_CSD_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CapSense_CSD_IntClock_PHASE & CapSense_CSD_IntClock_PHASE_MASK;
}
