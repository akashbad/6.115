/*******************************************************************************
* File Name: Target_UART_IntClock.c
* Version 2.0
*
*  Description:
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
void Target_UART_IntClock_Start(void) 
{
    ...
    Target_UART_IntClock_CLKEN |= Target_UART_IntClock_CLKEN_MASK;
	Target_UART_IntClock_CLKSTBY |= Target_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Target_UART_IntClock_Stop(void) 
{
    ...
    Target_UART_IntClock_CLKEN &= (uint8)(~Target_UART_IntClock_CLKEN_MASK);
	Target_UART_IntClock_CLKSTBY &= (uint8)(~Target_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Target_UART_IntClock_CLKEN & Target_UART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Target_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Target_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Target_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Target_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Target_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Target_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = Target_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Target_UART_IntClock_CLKEN & Target_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Target_UART_IntClock_DIV_PTR, clkDivider);
                Target_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Target_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Target_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Target_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Target_UART_IntClock_DIV_PTR, clkDivider);
				Target_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Target_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Target_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)Target_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Target_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Target_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Target_UART_IntClock_MOD_SRC & (uint8)(Target_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Target_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = Target_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Target_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        Target_UART_IntClock_MOD_SRC =
            (Target_UART_IntClock_MOD_SRC & (uint8)(~Target_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Target_UART_IntClock_MOD_SRC & Target_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Target_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Target_UART_IntClock_PHASE = clkPhase & Target_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Target_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Target_UART_IntClock_PHASE & Target_UART_IntClock_PHASE_MASK;
}
