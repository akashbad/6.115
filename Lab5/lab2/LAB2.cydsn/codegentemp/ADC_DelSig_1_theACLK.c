...
void ADC_DelSig_1_theACLK_Stop(void) 
{
    ...
    ADC_DelSig_1_theACLK_CLKEN &= (uint8)(~ADC_DelSig_1_theACLK_CLKEN_MASK);
	ADC_DelSig_1_theACLK_CLKSTBY &= (uint8)(~ADC_DelSig_1_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ADC_DelSig_1_theACLK_CLKEN & ADC_DelSig_1_theACLK_CLKEN_MASK) != 0u)
    {
...
#endif /* ADC_DelSig_1_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_DelSig_1_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_DelSig_1_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ADC_DelSig_1_theACLK_GetSourceRegister();
    uint16 oldDivider = ADC_DelSig_1_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_DelSig_1_theACLK_CLKEN & ADC_DelSig_1_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_DelSig_1_theACLK_DIV_PTR, clkDivider);
                ADC_DelSig_1_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_DelSig_1_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_DelSig_1_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_DelSig_1_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_DelSig_1_theACLK_DIV_PTR, clkDivider);
				ADC_DelSig_1_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ADC_DelSig_1_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_DelSig_1_theACLK_MOD_SRC |= modeBitMask & (uint8)ADC_DelSig_1_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ADC_DelSig_1_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_DelSig_1_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_DelSig_1_theACLK_MOD_SRC & (uint8)(ADC_DelSig_1_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ADC_DelSig_1_theACLK_GetDividerRegister();
    uint8 oldSrc = ADC_DelSig_1_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_DelSig_1_theACLK_MOD_SRC |= CYCLK_SSS;
        ADC_DelSig_1_theACLK_MOD_SRC =
            (ADC_DelSig_1_theACLK_MOD_SRC & (uint8)(~ADC_DelSig_1_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ADC_DelSig_1_theACLK_MOD_SRC & ADC_DelSig_1_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ADC_DelSig_1_theACLK_PHASE = clkPhase & ADC_DelSig_1_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_DelSig_1_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ADC_DelSig_1_theACLK_PHASE & ADC_DelSig_1_theACLK_PHASE_MASK;
}
