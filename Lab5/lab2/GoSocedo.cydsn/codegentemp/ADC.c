...
void ADC_Init(void) 
{
    ...
    ADC_DSM_DEM0_REG    = ADC_Pot_DSM_DEM0;
    ADC_DSM_DEM1_REG    = ADC_Pot_DSM_DEM1;
    ADC_DSM_MISC_REG    = ADC_Pot_DSM_MISC;
    ADC_DSM_CLK_REG    |= ADC_Pot_DSM_CLK;
    ADC_DSM_REF1_REG    = ADC_Pot_DSM_REF1;

    ADC_DSM_OUT0_REG    = ADC_Pot_DSM_OUT0;
    ADC_DSM_OUT1_REG    = ADC_Pot_DSM_OUT1;

    ADC_DSM_CR0_REG     = ADC_Pot_DSM_CR0;
    ADC_DSM_CR1_REG     = ADC_Pot_DSM_CR1;
#if(ADC_MI_ENABLE != 0u) /* Enable Modulator Input */
    ADC_DSM_CR3_REG     |= ADC_DSM_MODBIT_EN;
#else
    ADC_DSM_CR3_REG     = ADC_Pot_DSM_CR3;
#endif /* ADC_MI_ENABLE != 0u*/    
    ADC_DSM_CR8_REG     = ADC_Pot_DSM_CR8;
    ADC_DSM_CR9_REG     = ADC_Pot_DSM_CR9;
    ADC_DSM_CR13_REG    = ADC_Pot_DSM_CR13;

    ADC_DEC_SR_REG      = ADC_Pot_DEC_SR;

    /* Calculate Gain compensation coefficients for all configurations */
    ADC_GainCompensation(ADC_Pot_INPUT_RANGE,
                                      ADC_Pot_IDEAL_DEC_GAIN,
                                      ADC_Pot_IDEAL_ODDDEC_GAIN,
                                      ADC_Pot_RESOLUTION,
                                      ADC_Pot);
...
void ADC_Enable(void) 
{
    ...
    /* Code to disable the REFBUF0 if reference chosen is External ref */
    #if (((ADC_Pot_REFERENCE == ADC_EXT_REF_ON_P03) || \
          (ADC_Pot_REFERENCE == ADC_EXT_REF_ON_P32)) || \
        ...
        if (((config == 1u) &&
            ((ADC_Pot_REFERENCE == ADC_EXT_REF_ON_P03) ||
             (ADC_Pot_REFERENCE == ADC_EXT_REF_ON_P32))) ||
            ((config == 2u) &&
            ((ADC_CFG2_REFERENCE == ADC_EXT_REF_ON_P03) ||
             (ADC_CFG2_REFERENCE == ADC_EXT_REF_ON_P32))) ||
    ...
    #if (((ADC_Pot_INPUT_RANGE == ADC_IR_VSSA_TO_2VREF) || \
         ((ADC_DEFAULT_NUM_CONFIGS > 1) && \
        ...
        if(((config == 1u) &&
            (ADC_Pot_INPUT_RANGE == ADC_IR_VSSA_TO_2VREF) &&
            ((ADC_Pot_REFERENCE != ADC_EXT_REF_ON_P03) &&
             (ADC_Pot_REFERENCE != ADC_EXT_REF_ON_P32))) ||
             ((config == 2u) &&
              (ADC_CFG2_INPUT_RANGE == ADC_IR_VSSA_TO_2VREF) &&
    ...
    ADC_DSM_CR2_REG = ADC_Pot_DSM_CR2;
...
int16 ADC_CountsTo_mVolts(int32 adcCounts) 
{
    ...
    #if(ADC_Pot_DEC_DIV != 0)
        if(ADC_Config == ADC_Pot)
        {
            adcCounts /= ADC_Pot_DEC_DIV;
        }
    #endif /* ADC_Pot_DEC_DIV */
...
int32 ADC_CountsTo_uVolts(int32 adcCounts) 
{
    ...
    switch (resolution)
    {
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_12) || \
            ...
            case (uint8)ADC__BITS_12:
                ...
                break;
        #endif /* ADC__BITS_12 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_13) || \
            ...
            case (uint8)ADC__BITS_13:
                ...
                break;
        #endif /* ADC__BITS_13 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_14) || \
            ...
            case (uint8)ADC__BITS_14:
                ...
                break;
        #endif /* ADC__BITS_14 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_15) || \
            ...
            case (uint8)ADC__BITS_15:
                ...
                break;
        #endif /* ADC__BITS_15 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_16) || \
            ...
            case (uint8)ADC__BITS_16:
                ...
                break;
        #endif /* ADC__BITS_16 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_17) || \
            ...
            case (uint8)ADC__BITS_17:
                ...
                break;
        #endif /* ADC__BITS_17 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_18) || \
            ...
            case (uint8)ADC__BITS_18:
                ...
                break;
        #endif /* ADC__BITS_18 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_19) || \
            ...
            case (uint8)ADC__BITS_19:
                ...
                break;
        #endif /* ADC__BITS_19 */    
        #if( (ADC_Pot_RESOLUTION == ADC__BITS_20) || \
...
static void ADC_InitConfig(uint8 config) 
{
    ...
    {
        /* Default Config */
        ADC_DEC_CR_REG      = ADC_Pot_DEC_CR;
        ADC_DEC_SHIFT1_REG  = ADC_Pot_DEC_SHIFT1;
        ADC_DEC_SHIFT2_REG  = ADC_Pot_DEC_SHIFT2;
        ADC_DEC_DR2_REG     = ADC_Pot_DEC_DR2;
        ADC_DEC_DR2H_REG    = ADC_Pot_DEC_DR2H;
        ADC_DEC_DR1_REG     = ADC_Pot_DEC_DR1;
        ADC_DEC_OCOR_REG    = ADC_Pot_DEC_OCOR;
        ADC_DEC_OCORM_REG   = ADC_Pot_DEC_OCORM;
        ADC_DEC_OCORH_REG   = ADC_Pot_DEC_OCORH;
        ADC_DEC_COHER_REG   = ADC_Pot_DEC_COHER;

        ADC_DSM_CR4_REG     = ADC_Pot_DSM_CR4;
        ADC_DSM_CR5_REG     = ADC_Pot_DSM_CR5;
        ADC_DSM_CR6_REG     = ADC_Pot_DSM_CR6;
        ADC_DSM_CR7_REG     = ADC_Pot_DSM_CR7;
        ADC_DSM_CR10_REG    = ADC_Pot_DSM_CR10;
        ADC_DSM_CR11_REG    = ADC_Pot_DSM_CR11;
        ADC_DSM_CR12_REG    = ADC_Pot_DSM_CR12;
        ADC_DSM_CR14_REG    = ADC_Pot_DSM_CR14;
        ADC_DSM_CR15_REG    = ADC_Pot_DSM_CR15;
        ADC_DSM_CR16_REG    = ADC_Pot_DSM_CR16;
        ADC_DSM_CR17_REG    = ADC_Pot_DSM_CR17;
        /* Set DSM_REF0_REG by disabling and enabling the PRESS circuit */
        ADC_SetDSMRef0Reg(ADC_Pot_DSM_REF0);
        ADC_DSM_REF2_REG    = ADC_Pot_DSM_REF2;
        ADC_DSM_REF3_REG    = ADC_Pot_DSM_REF3;

        ADC_DSM_BUF0_REG    = ADC_Pot_DSM_BUF0;
        ADC_DSM_BUF1_REG    = ADC_Pot_DSM_BUF1;
        ADC_DSM_BUF2_REG    = ADC_Pot_DSM_BUF2;
        ADC_DSM_BUF3_REG    = ADC_Pot_DSM_BUF3;
        ...
        */
        #if(ADC_Pot_RESOLUTION > 16 && \
            ADC_Pot_CONV_MODE == ADC_MODE_SINGLE_SAMPLE)
            ADC_stopConversion = 1u;
        #endif /* Single sample with resolution above 16 bits. */

        ADC_CountsPerVolt = (int32)ADC_Pot_COUNTS_PER_VOLT;

        ADC_Ext_CP_Clk_SetDividerRegister(ADC_Pot_CP_CLK_DIVIDER, 1u);

        /* This is only valid if there is an internal clock */
        #if(ADC_DEFAULT_INTERNAL_CLK)
            ADC_theACLK_SetDividerRegister(ADC_Pot_ADC_CLK_DIVIDER, 1u);
        #endif /* ADC_DEFAULT_INTERNAL_CLK */
